...
function notAllowed(req, res) {
  res.sendHeader(500, [ ["Content-Type", "text/plain"]
                      , ["Content-Length", NOT_ALLOWED.length]
                      ]);
  res.write(NOT_ALLOWED);
  res.end();
}
...
fu.createServer = function(www_path) {
  return createServer(function (req, res) {
    if (req.method === "GET" || req.method === "HEAD") {
      var handler = getMap[url.parse(req.url).pathname] || fu.staticHandler(www_path + req.url);
...
fu.staticHandler = function (filename) {
  ...
  sys.debug("filename: " + filename);

  function loadResponseData(callback) {
    sys.puts("loadResponseData " + filename + "...");
